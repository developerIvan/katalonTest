...
//Descripci?n del sub deporte
String gameDesc = ''

//Define el tipo de sub deporte, puede ser prop o game
String oddGameType = ''

//Descripci?n del prop del deporte asociado
...
String betSlipButtonDesc = '';

//-----------General variables --------------------//
Map<String, String> oddGameTypesMap = GlobalVariable.oddDescpirtion

Map<String, String> buttonLanguageMap = GlobalVariable.buttonDescMap

String expectedButtonDesc = '';
...
//------------------------Sports selectors-------------------------------//
String subSportSelector = '.sportsPanelWrap div:nth-child(1) .panel-group ul li div.table div div.ellipsis'
...
String linebuttonCSSSelector =gameNumAwayTeamCSSSelector.concat(' button[gamenum="?"]') ;
//-----------betslip selectors----------------//
// Css
...
String betSlipMaxLimitSelector = betSlipBodySelector.concat(EMPTY_SPACE).concat('div.maxbet div.limit')

String betSlipIdButtonSelector = 'placeBet';
//-----------betslip objects ------------------------------//
...
try {
	...
	if (gameDesc.equalsIgnoreCase(oddGameType)) {
		...
		//Comparaci?n del encabezado del betSlip con el encabezado de los proprs del sub deporte
		...
		//Comparaci?n para asegurar que la descipci?n de la apuesta con la descipci?n de contestant  seleccioando
		...
		//comparaci?n de la l?nea del betslip sea igual a la l?nea seleccionada en el contestant
		...
		//Valida que la descripci?n del betslip sea igual al encabezado del contest de torneo
		assert betSlipContestDesc.equals(futurePropContestName)


	} else {
		//validaci?n de lineas  del juego
		...
		awayTeamDesc = CustomKeywords.'com.utils.AutomationUtils.getObjectAttribute'("Team Home Desc", 'innerText', new TestObjectProperty(CSS_SELECTOR, equalsCondType, awayTeamDescCSSSelector));
		...
		//Valdiaci?n de equipo seleccionado
		...
		//Validaci?n si l?nea es de tipo spread
		if('Spread'.equalsIgnoreCase(lineType)){
			threshold  =  CustomKeywords.'com.utils.AutomationUtils.getObjectAttribute'("Threshohd Object", textContentAtribute, new TestObjectProperty(CSS_SELECTOR, equalsCondType,linebuttonCSSSelector+' div.row [class^=threshold]' ));
		}
		...
		lineValue = CustomKeywords.'com.utils.AutomationUtils.getObjectAttribute'("Odd Object", textContentAtribute, new TestObjectProperty(CSS_SELECTOR, equalsCondType,linebuttonCSSSelector+' div.row [class^=odds]' ));;
		...
		if(!'Money'.equalsIgnoreCase(lineType)){
			String palyerBalance = CustomKeywords.'com.utils.AutomationUtils.getObjectAttribute'("Customer Balance", textContentAtribute, new TestObjectProperty(CSS_SELECTOR, equalsCondType,'span.AvailableBalance' ));

			String betslipMaxAmount =  CustomKeywords.'com.utils.AutomationUtils.getObjectAttribute'("BetSlip max limit", textContentAtribute, new TestObjectProperty(CSS_SELECTOR, equalsCondType,betSlipMaxLimitSelector));

			//Comparaci?n con el balance
			assert betslipMaxAmount.equals(palyerBalance);
		}
	}


	//Valida bot?n del betSlip
	betSlipButtonDesc = CustomKeywords.'com.utils.AutomationUtils.getObjectAttribute'("BetSlip Button", textContentAtribute, new TestObjectProperty('id', equalsCondType, betSlipIdButtonSelector));

	expectedButtonDesc = buttonLanguageMap.get(languageKey);
	
	assert betSlipButtonDesc != null;
	
	assert expectedButtonDesc != null;
	
	assert betSlipButtonDesc.equalsIgnoreCase(expectedButtonDesc);
	...
	//Guara descripci?n de la prueba
	CustomKeywords.'com.utils.ExcelsUtils.saveDataOnExcel'(1, 13, 'Validacion de permiso fallida por incumplimiento de verifaicaci?n de elementos en la p?gina')

	throw new AssertionError('Error en la prueba revisi?n de l?neas debido a que hay un paso que no se cumplio', step)
}
catch (Exception ex) {
	//Guara estado de la prueba
	CustomKeywords.'com.utils.ExcelsUtils.saveDataOnExcel'(1, 12, log.FAILED)
	...
	CustomKeywords.'com.utils.ExcelsUtils.saveDataOnExcel'(1, 13, 'Validacion de permiso fallida causado por excepci?n inesperada')
