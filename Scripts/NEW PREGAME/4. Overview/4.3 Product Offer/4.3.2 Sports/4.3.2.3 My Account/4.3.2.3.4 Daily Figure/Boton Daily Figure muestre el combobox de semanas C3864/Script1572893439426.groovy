...
try {
	...
	//Se verifica que el combobox de semanas es visible
	WebUI.waitForElementVisible(findTestObject('Repositorio Objetos Proyecto Premium/4. Overview/4.3 Product Offer/4.3.2 Sports/4.3.2.3 MY ACCOUNT/4.3.2.3.4 Daily Figure/select_DailyFigureWeeks'),
			2)

	WebUI.verifyElementVisible(findTestObject('Repositorio Objetos Proyecto Premium/4. Overview/4.3 Product Offer/4.3.2 Sports/4.3.2.3 MY ACCOUNT/4.3.2.3.4 Daily Figure/select_DailyFigureWeeks'))

	testStatus = 'Exitoso'

	testResultDescription = 'El combobox de semanas es visible'
}
	...
	testResultDescription = ('El combobox de semanas no es visible  debido a que un paso de la prueba no se completo o un elemento de la página que no está visible. ' +
...
/*catch (Exception e) {
	...
	testResultDescription = ('El combobox de semanas no es visible debido a un error anomalo en la prueba. ' + CustomKeywords.'com.utils.ConstantsUtil.getCustomErrorMessageForGeneralExceptions'(
			errorCode))
