...
String number = dailyFigureWeekNumber
try {
    ...
    //Valida precondicion 2 de caso de prueba
    ...
    //Valida precondicion 3 de caso de prueba
	...
	KeywordLogger.getInstance(this.class).logger.info("parametro de día "+dailyFigureWeekNumber);
    ...
    dailyFigureWeek = WebUI.callTestCase(findTestCase('NEW PREGAME/4. Overview/4.3 Product Offer/4.3.2 Sports/4.3.2.3 My Account/4.3.2.3.4 Daily Figure/Funciones Auxiliares/SeleccionarSemana'), 
        [('weekNumber') :Integer.parseInt(number)], FailureHandling.STOP_ON_FAILURE)
	...
	container.setDailyFigureDate(dailyFigureWeek);
	
	container.setDailyFgureDay(dayOfWeek);
	
    testStatus = 'Exitoso'

    testResultDescription = ((((('El reporte de Daily Figure muestra satisfactoriamente los rubros jugador ' + customerPIN) + 
    ' del dia   ') + dailyFigureDay) + ' y la semana ') + dailyFigureWeek)

	GlobalVariable.precondicionC6400Completa = true;
	GlobalVariable.dailyFigureContainer = container;
    return container
}
/*catch (com.kms.katalon.core.exception.StepFailedException stepE) {
    ...
    testResultDescription = ((((('El jugador ' + customerPIN) + ' no tiene rubros  para el día ') + dailyFigureDay) + '  o o algún procedimiento previo como ingreso a pregame no se pudo completar ') + 
    ...
    testResultDescription = (('El jugador ' + customerPIN) + ' no tiene rubros para el día seleccionado, favor revisar el log de katalon ')
