...
try {
	...
	WebElement periodButton = WebUI.callTestCase(findTestCase('NEW PREGAME/5. Betslip/5.1 Bet Type/Funciones Auxiliares/SeleccionarPeriodoDelJuego'),
		[('sportName') : sportId, ('subSportName') : subSporId, ('leagueName') : tournament, ('periodNumber') : gamePeriod],
		FailureHandling.STOP_ON_FAILURE)
	...
	WebElement selectedLineButton = WebUI.callTestCase(findTestCase('NEW PREGAME/5. Betslip/5.1 Bet Type/Funciones Auxiliares/SeleccionarLineaAlt'),
		[('betType') : 'M', ('selectedTeam') : favoriteTeam, ('tournament') : tournament, ('period') : gamePeriod], FailureHandling.STOP_ON_FAILURE)
	...
	//Se prepara la información esperada (Matach,Periodo,Bet type, Selectiony Line etc) de la línea seleccioanda
	...
	betslipActualResult = CustomKeywords.'com.utils.BetSlipUtils.returnWagerGeneralInfo'(gameNum, gamePeriod)

	assert betslipExpectedResult.contains(betslipActualResult)

	testStatus = 'Exitoso'

	testResultDescription = (('La siguiente información de la línea   ' + betslipExpectedResult) + '  con moneyline se refleja correctamente en el betslip')
