...
String expectedDailyFigureDays = resultadoEsperado;
String actualDailyFigureDays = '';
...
try {
    ...
    TestObject dailyFigureDaysTable = new TestObject("Daily Figure Days");
	dailyFigureDaysTable.setSelectorMethod(SelectorMethod.CSS)
	dailyFigureDaysTable.setSelectorValue(SelectorMethod.CSS, "tr.trReportDetail")
	
	WebUI.waitForElementVisible(dailyFigureDaysTable, 2)

	actualDailyFigureDays = WebUI.getAttribute(dailyFigureDaysTable, "innerText") 

	actualDailyFigureDays = actualDailyFigureDays.replaceAll("\\s+",",");
	
	assert expectedDailyFigureDays.equals(actualDailyFigureDays);
...
/*catch (StepFailedException stepE) {
    ...
    testResultDescription = "La tabla de días de daily figure debería desplegar los siguientes días: "+ expectedDailyFigureDays + ' pero actualmente nuesta estos ' + actualDailyFigureDays
