...
try {
	...
	//Se selecciona el subDeporte
	WebUI.callTestCase(findTestCase('NEW PREGAME/5. Betslip/5.1 Bet Type/Funciones Auxiliares/SeleccionarSubDeporte'), [('sportName') :sportId
		, ('subSportName') : subSporId], FailureHandling.STOP_ON_FAILURE)
    ...
    //Selecciono la línea
    WebElement selectedLineButton = WebUI.callTestCase(findTestCase('NEW PREGAME/5. Betslip/5.1 Bet Type/Funciones Auxiliares/SeleccionarLineaAlt'), 
        [('betType') : 'S', ('selectedTeam') : favoriteTeam, ('tournament') : tournament, ('period') : period], FailureHandling.STOP_ON_FAILURE)
    ...
    expectedMatch = WebUI.callTestCase(findTestCase('NEW PREGAME/5. Betslip/5.1 Bet Type/Funciones Auxiliares/DefinirMatchExperado'), 
        [('gameNum') : gameNum], FailureHandling.STOP_ON_FAILURE)
    ...
    //Valida que el odd sea negativo como lo indica la prueba
    assert odds.contains('-')

    assert betslipExpectedResult.contains(betslipActualResult)

    testStatus = 'Exitoso'

    testResultDescription = (('La siguiente información de la línea   ' + betslipExpectedResult) + '  con spread asian -0.75 se refleja correctamente en el betslip o el odd es positivo')
    ...
    testResultDescription = ((('La información esperada del betslip era esta: ' + betslipExpectedResult) + ' pero actualmente es: ') + 
    betslipActualResult)
    ...
    testResultDescription = ('No se pudo verificar que la informaicón se refleje correctamente con un asian handicap -0.75  debido a un error anomalo en la prueba. ' + 
