...
public class AutomationUtils {
	...
	/**
	 * Funcion para dar formato a los montos de los rubros que tengan solo un decimal para pasarlo a dos cemilaes y omologarlos al formato que tiene cuistomer maitnenance
	 * @param d
	 * @return
	 */
	@Keyword
	def String roundDoubleToFourDecimals(double d){
		String formatedDouble = String.valueOf(d);

		String decimalsFromDuble = formatedDouble.substring(formatedDouble.indexOf(".")+1,formatedDouble.length())

		if(decimalsFromDuble.length()==1){
			formatedDouble = formatedDouble.concat("0");
		}
		return formatedDouble;
	}

	@Keyword
	def void resetGlobalVariables(){
		GlobalVariable.botonDailyFigureEsPresionado = false;
		GlobalVariable.browserIsOpen = false;
		GlobalVariable.usuarioLogeado = false;
		GlobalVariable.dailyFigureContainer = null;
